 ...
 * Android WebViews, iOS Safari, and desktop browsers without ANY dependency on CSS parsers.
 */
export const renderInvoiceToCanvas = (
  ...
  return new Promise((resolve) => {
    ...
    const totalHeight = baseHeight + sale.items.length * itemRowHeight;
    ...
    ctx.fillText(`Phone: ${shopDetails.phone || ''} ${shopDetails.gstin ? '| GSTIN: ' + shopDetails.gstin : ''}`, width / 2, 88);
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`Invoice: ${sale.invoiceNumber}`, 24, headerHeight + 25);
    ...
    ctx.fillText(`Date: ${sale.date}`, width - 24, headerHeight + 25);
    ...
    sale.items.forEach((item, idx) => {
      ...
      ctx.font = '13px system-ui, -apple-system, sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText(`${idx + 1}. ${item.vegEmoji || '🥬'} ${item.vegName}`, 32, currentY + 23);

      ctx.textAlign = 'right';
      ctx.fillText(`${item.quantity.toFixed(2)}`, 340, currentY + 23);
      ctx.fillText(`₹${item.pricePerKg.toFixed(1)}`, 460, currentY + 23);
      
      ctx.font = 'bold 13px system-ui, -apple-system, sans-serif';
      ctx.fillStyle = '#0f172a';
      ctx.fillText(`₹${item.total.toFixed(1)}`, width - 36, currentY + 23);
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`₹${sale.totalAmount.toFixed(1)}`, width - 40, currentY + 30);
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`₹${sale.totalAmount.toFixed(1)}`, width - 40, currentY + 62);
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`₹${sale.amountPaid.toFixed(1)}`, width - 40, currentY + 92);
    ...
    ctx.textAlign = 'center';
    ctx.fillText(`Payment Mode: ${(sale.paymentMethod || 'CASH').toUpperCase()} | Generated from VEGI BILLING APP`, width / 2, currentY);
